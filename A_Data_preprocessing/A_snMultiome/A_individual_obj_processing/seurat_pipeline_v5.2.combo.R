####2022-03-28
# Now start with raw matrix. Alla added incorporation of Scrublet annotaion from Austin's combo scrublet pipeline
# Alla added SCTtransform now regresses out cell cycle scores
# Modified by Alla to add removal of peaks with unknown nucleotides in their sequence plus put back blacklist ratio filtering 2022-02-10
# Modified by Alla to fix iterative removal procedure, add functions, add data.table usage for faster processing 2021-08-17
...
if (!is.null(scrub.folder)){
  cat('add scrublet\n')
  scrub.path <- list.files(path = scrub.folder, pattern = '.csv', full.names = T)
  print(scrub.path)
  scrub.table <- fread(scrub.path, header = T) %>% data.frame(row.names = 1)
  scrub.table$predicted_doublet <- ifelse(scrub.table$predicted_doublet_atac=='True' & scrub.table$predicted_doublet_rna=='True', 'True', 'False')
...
blacklist_counts <- CountsInRegion(panc, assay = 'ATAC_MACS2', region = blacklist_hg38_unified) %>% data.frame(); colnames(blacklist_counts) <- 'blacklist_region_fragments'
...
message('doing FragmentHistogram')
fragment_period_plot=FragmentHistogram(object = panc, group.by = 'nucleosome_group', region = "chr1-1-10000000")	
message('done')
...
panc$percent.mt[panc$percent.mt=='NaN'] <- NA
print(head(panc@meta.data))
QCplot <- VlnPlot(object = panc, 
                  features = c('nFeature_RNA', 'nCount_RNA'), 
                  ncol = 3)

pdf(paste(outputpath,"/",sample,"_0_RNA_QC.pdf",sep=""),height=5,width=11)
print(QCplot)
dev.off()
...
VlnPlot(object = panc, features = c('pct_read_in_peaks_500MACS2', 'peak_region_fragments_500MACS2',  'TSS.enrichment', 'nucleosome_signal', "nFeature_RNA", "nCount_RNA",'percent.mt'), ncol = 7)
dev.off()
